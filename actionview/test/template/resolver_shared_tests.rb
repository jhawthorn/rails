module ResolverSharedTests
  ...
  def test_can_find_with_just_format
    with_file "test/hello_world.text", "Hello text!"

    templates = resolver.find_all("hello_world", "test", false, locale: [:en], formats: [:text], variants: [:phone], handlers: [:erb])
    assert_equal 1, templates.size
    assert_equal "Hello text!",   templates[0].source
    assert_equal "test/hello_world", templates[0].virtual_path
